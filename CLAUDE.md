...
## Data Flow
Tasks load from `data/tasks.json` on page open (cache-busted). The 'From your inbox' panel loads AI task suggestions from work-inbox `data/inbox_suggestions.json` (generated by fetch_inbox.py Phase 3.5, six times daily); accept by telling Claude, dismissals persist in localStorage. Done-state persists in `localStorage` (key: `commandCentre_done_v1`). Manual tasks added via quick-add panel exist in memory only — lost on refresh. Persistent manual tasks require GitHub write-back (ROADMAP item 1).
