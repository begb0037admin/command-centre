...
- **Backup-and-verify:** `Archive/{index,styles,app,api}_backup_20260924_0848.*` committed (`9705a2f`) and SHA-verified via the Contents API before any edit; the merged `index.html`, `css/styles.css`, `js/app.js`, `js/api.js` and `js/vendor/Sortable.min.js` blob SHAs match the Contents API. The served `app.js` and `styles.css` are byte-identical to the merged blobs.
- **Live version:** `ca5a1dd6-3b7a-48f7-a8a6-77f6a2bf31d2`. **Rollback:** `npx wrangler rollback 5f946874-75de-46f9-ab05-7a5c930f1913 --name command-centre` (pre-change code), then revert PR #16 on main so the next auto-deploy doesn't re-ship it; or restore the four files from the Archive backups above.
- **What changed:** SortableJS (vendored 1.15.6) replaces the HTML5 task drag, with a ghost card following the pointer, a placeholder gap, touch support and serial saves through the existing `persistTasks()` path to cc-tasks-writer. The Worker is unchanged. Moves show "Moved to X" with Undo; on a failed save the board reloads and says so. **Moves are across tiers only:** within-tier order stays source-date sorted (Kevin's 1 Sep decision), so within-list reordering is off rather than showing a misleading landing spot. The done "dot" is removed. Each card now has Edit (rename), Archive (`done=true`, server-side, existing done-sync; on done cards this becomes Restore) and Delete, all with Undo. Delete, the drawer Delete and log-entry removal use an in-page confirm; no native `confirm()` is left. "Show done" is the archived view. Every card has a chevron that toggles the drawer, with state remembered in `cc_expanded_v1`, keyboard support and "Expand all / Collapse all" per tier. The inbox-suggestion native drag onto a tier and the "+ Today/Tomorrow/Week" buttons still work.
- **Verified pre-deploy:** real Chromium at 1440px, with the task store and Worker mocked so there were no live writes. Covered: cross-tier drag (white fallback clone, placeholder in the target list), Undo, rapid double move saved serially, drag by card body, within-tier drag makes no write, save failure reloads with a message, touch drag, archive/undo/restore, delete confirm/undo (same index)/cancel, expand persistence, keyboard, expand-all, blocked storage, and suggestion +Today plus the native drop. `node --check` passes; `tests/staleness_parity_test.js` passes.
- **Verified live:** created a throwaway task "ZZ Drew live check" through the app's own path. Tested a real drag Tomorrow→Today with the ghost visible, Undo, persistence after reload, Archive+Undo, Archive→Show done→Restore, and expanding a real card. Then deleted the task permanently (in-page confirm). It's gone from `tasks.json`. Done-sync left an inert `"id_task-1790236948396": false` key in work-inbox `data/ticks.json`; that's the existing Worker's normal undone representation.
- **Decision for Kevin:** he was reordering cards *within* tiers on 23 Sep ("Reorder:" commits), but the 1 Sep source-date sort always overrides that. If he wants manual within-tier order, source-date sorting has to be dropped. That's his call; it's not built.
